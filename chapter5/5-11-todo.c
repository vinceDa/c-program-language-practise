...
/**
 * 练习5-11 修改程序 entab 和 decab（第1 章练习中编写的函数)，使它们接受一组作为参数的制表符停止位。如果启动程序时不带参数，则使用默认的制表符停止位设置。
 * 练习 5-12 对程序 entab 和 detab 的功能做一些扩充，以接受下列缩写的命令：entab -m +n 表示制表符从第m列开始，每隔n列停止。
 * 选择（对使用者而言）比较方便的默认行为。
